...
const ChecckBox = ({label, selected = false, onChange = () => { }})=>{
    return (
        <label className="flex gap-3 items-center cursor-pointer mt-2 text-sm">
            <input type="checkbox" checked={selected} onChange={(e)=>onChange(e.
            target.checked, label)}/>
                <span className='fontlight select-none'>{label}</span>
        </label>
    )
}
        ...
        xl:px-32'>
            <div>
                <div className='flex flex-col items-start text-left'>
                    ...
                    max-w-174'>Take advantage ofm our limited-time offers and special packages to 
                ...
                {roomsDummyData.map((room)=>(
                    ...
                    border-b border-gray-300 last:pb-30 last:border-0'>
                        ...
                        className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover // rounded-x1 → rounded-xl
                        cursor-pointer' />
                        <div className='md:w-1/2 flex flex-col gap-2'>
                            ...
                            <div>
                                {room.amenities.map((item, index)=>(
                                    ...
                                    px-3 py-2 rounded-lg bg-[#F5F5FF]/70'>           <img src={facilityIcons[item]} alt={item}
                                    className='w-5 h-5' />
                                    <p> {item}</p>
                                    </div>
                                ))}
            ...
            {/* fillter */}
            <div className='bg-white w-80 border-gray-300 text-gray-600
